...
pub use stats::RunningDbStats;

use std::{cmp, collections::HashMap, error, fs, io, mem, path::Path, result};

use parking_lot::{Mutex, MutexGuard, RwLock};
use rocksdb::{
    BlockBasedOptions, CFHandle, ColumnFamilyOptions, DBOptions, ReadOptions, Writable, WriteBatch,
    WriteOptions, DB,
};
...
use parity_util_mem::{
    MallocSizeOf as ParityMallocSizeOf, MallocSizeOfOps as ParityMallocSizeOfOps,
};
#[cfg(target_os = "linux")]
use regex::Regex;
#[cfg(target_os = "linux")]
...
impl Database {
    ...
    pub fn open(config: &DatabaseConfig, path: &str) -> io::Result<Database> {
        ...
        for i in 0..config.columns {
            cf_options.push((cfnames[i as usize], col_config(&config, &block_opts)?));
        }
        ...
        let db = match db {
            ...
            Err(ref s) if is_corrupted(s) => {
                ...
                DB::repair(opts.clone(), path).map_err(other_io_err)?;
                let db = DB::open_cf(opts.clone(), path, cf_options).map_err(other_io_err)?;
                for name in cfnames {
                    let _ = db
                        .cf_handle(name)
                        .expect("rocksdb opens a cf_handle for each cfname; qed");
    ...
    fn write_flushing_with_lock(&self, _lock: &mut MutexGuard<'_, bool>) -> io::Result<()> {
        match *self.db.read() {
            Some(ref cfs) => {
                ...
                self.stats.tally_writes(stat_ops as u64);
                self.stats.tally_bytes_written(stat_bytes as u64);
    ...
    pub fn get(&self, col: u32, key: &[u8]) -> io::Result<Option<DBValue>> {
        match *self.db.read() {
            Some(ref cfs) => {
                ...
                let value = match overlay.get(key) {
                    ...
                    None => {
                        ...
                        match flushing.get(key) {
                            ...
                            None => cfs
                                .db
                                .get_cf_opt(cfs.get_cf(col as usize), key, &self.read_opts)
                ...
                self.stats.tally_bytes_read(stat_bytes as u64);
                Ok(value)
            }
            None => Ok(None),
        }
    ...
    pub fn add_column(&self) -> io::Result<()> {
        match *self.db.write() {
            Some(DBAndColumns {
                ...
                db.create_cf((name.as_str(), col_config(&self.config, &self.block_opts)?))
                    .map_err(other_io_err)?;
    ...
    #[allow(dead_code)]
    fn dev_print(stats: HashMap<String, RocksDbStatsValue>) {
        fn has_prefix(content: &str, prefix: &[u8]) -> bool {
            let len = prefix.len();
            content.as_bytes().len() >= len && content.as_bytes()[0..len].as_ref() == prefix
        }

        for (key, value) in stats.iter() {
            // if !(has_prefix(key, b"block.cache.") || has_prefix(key, b"number.")) {
            //     continue;
            // }
            if value.times.is_none() || value.times.unwrap().sum == 0 {
                if value.count != 0 {
...
mod tests {
    ...
    fn test_memory_property() {
        let tempdir = TempDir::new("").unwrap();
        let db =
            Database::open(&DatabaseConfig::default(), tempdir.path().to_str().unwrap()).unwrap();
        let key1 =
            H256::from_str("02c69be41d0b7e40352fc85be1cd65eb03d40ef8427a0ca4596b1ead9a00e9fc")
                .unwrap();
