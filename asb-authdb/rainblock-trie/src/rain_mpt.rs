...
struct SearchResult {
    stack: Vec<(MptNode, Option<Nibble>)>,
    matched: bool,
    remainder: Vec<Nibble>,
}

impl SearchResult {
    fn matched(&self) -> Option<&MptNode> {
        if self.matched {
    ...
    fn matched_mut(&mut self) -> Option<&mut MptNode> {
        if self.matched {
...
impl<const N: usize> MerklePatriciaTree<N> {
    pub fn new(db: Arc<dyn KeyValueDB>) -> MerklePatriciaTree<N> {
        let root = if let Some(digest) = db.get(0, &ROOT_KEY).expect("Cannot load db") {
            ...
            let node = Rc::new(TrieNodeExt::load(&db, digest));
            node.load_children::<N>(&db, 0);
            Some(node)
        } else {
            None
        };
    ...
    pub fn get(&mut self, key: Vec<u8>) -> Option<Vec<u8>> {
        ...
        if let Some(matched_node) = result.matched_mut() {
            if let Some(value) = matched_node.value() {
    ...
    pub fn put(&mut self, key: Vec<u8>, val: Vec<u8>) {
        ...
        if let Some(matched_node) = result.matched() {
            if matched_node.value().unwrap() == &val {
                // Value not changed
                return;
            }
        }
    ...
    pub fn flush_all(&mut self) -> io::Result<()> {
        let mut put_ops = Vec::new();
        if let Some(root) = &self.root {
            root.commit::<N>(0, &mut put_ops, true);
        }
    ...
    // Reset the pointers to the child which will be changed. This can save memory cost in Rc::make_mut
    fn reset_pointers(&mut self, stack: &mut Vec<(Rc<TrieNodeExt>, Option<Nibble>)>) {
        self.root = None;
        for (node, child_idx) in stack.iter_mut() {
            if let Some(&child_idx) = child_idx.as_ref() {
                let trie_node = TrieNodeExt::make_mut(node, &mut self.del_ops);
                *trie_node.child_mut(child_idx).unwrap() = ChildRef::Null;
            }
        }
    }

    fn recover_pointers(&mut self, stack: Vec<(Rc<TrieNodeExt>, Option<Nibble>)>) {
        let mut last_child: Option<Rc<TrieNodeExt>> = None;
        for (mut node, child_idx) in stack.into_iter().rev() {
            if let Some(&child_idx) = child_idx.as_ref() {
                let trie_node = TrieNodeExt::make_mut(&mut node, &mut self.del_ops);
...
impl<const N: usize> MerklePatriciaTree<N> {
    ...
    fn insert_on_branch(
        ...
        match TrieNodeExt::make_mut(prev_branch_node.owned_mut().unwrap(), &mut self.del_ops) {
        ...
        if !ext_key_rest.is_empty() {
            ...
            if let TrieNode::Branch { .. } = &***child_node {
                ...
                *child = ChildRef::Owned(
                ...
                let key = TrieNodeExt::make_mut(child_node, &mut self.del_ops)
                    .key_mut()
                    .unwrap();
...
impl<const N: usize> MerklePatriciaTree<N> {
    fn remove(&mut self, stack: &mut Vec<(Rc<TrieNodeExt>, Option<Nibble>)>) {
        // #[cfg(test)] {
        //     println!("Show stack");
        ...
        let last_node = TrieNodeExt::make_mut(&mut last, &mut self.del_ops);
        match last_node {
            ...
            TrieNode::Leaf { .. } => {
                ...
                let last_parent_mut = TrieNodeExt::make_mut(&mut last_parent, &mut self.del_ops);
                let idx = idx.unwrap();
    ...
    fn drop_single_child_branch(&mut self, idx: Nibble, mut child_ref: ChildRef) -> TrieNode {
        ...
        if child.key().is_some() {
            // leaf or extension
            let child = TrieNodeExt::make_mut(child, &mut self.del_ops);
    ...
    fn push_non_branch_to_stack(
        &mut self,
        mut node: TrieNode,
        stack: &mut Vec<(Rc<TrieNodeExt>, Option<Nibble>)>,
    ) {
        if let Some((last_parent, idx)) = stack.pop() {
            if let TrieNode::Extension { key: ext_key, .. } = &**last_parent {
