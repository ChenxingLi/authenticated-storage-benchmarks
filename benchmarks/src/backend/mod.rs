...
pub fn backend(opts: &Options) -> Arc<dyn KeyValueDB> {
    match opts.backend {
        ...
        crate::opts::Backend::InMemoryDB => {
            Arc::new(kvdb_memorydb::create(opts.num_cols()))
        },
        crate::opts::Backend::MDBX => {
            Arc::new(mdbx::open_database(opts))
        },
    }
}
